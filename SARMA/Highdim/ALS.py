...
def ALS(y,p,r,s,r1,r2,N,T,P,n_iter,lr_omega,lmbd_true,gamma_true,theta_true,true_A,true_G,stop_thres):
    # Initialization
    d = p+r+2*s
    Loss = np.inf
    ...
    for iter_no in range(n_iter):
        ...
        for k in range(r):
            ...
            lmbd[k] = max(min(0.99,lmbd[k]),-0.99)
        ...
        # early stop
        # if (np.linalg.norm(tensor_op.unfold(G-pre_G,1), ord='fro') < stop_thres * np.linalg.norm(tensor_op.unfold(true_G,1), ord='fro')) and (np.linalg.norm(np.concatenate([lmbd-pre_lmbd,gamma-pre_gamma,theta-pre_theta]), ord=2) < stop_thres * np.linalg.norm(np.concatenate([lmbd_true,gamma_true,theta_true]), ord=2)):
        # # if (np.linalg.norm(tensor_op.unfold(G-pre_G,1), ord='fro') < stop_thres) and (np.linalg.norm(np.concatenate([lmbd-pre_lmbd,gamma-pre_gamma,theta-pre_theta]), ord=2) < 0.1*stop_thres):
        #     Loss = loss(y,G,L,T)
