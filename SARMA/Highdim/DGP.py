...
def DGP_TIME(N,T,burn,p,r,s):
    ...
    # generate lmbd
    if r > 0:
        lmbd_full = [-0.8,-0.5,-0.2,0.2,0.5,0.8]
        lmbd = lmbd_full[:r]
        J = np.diag(lmbd)
    
    # generate gamma,theta pair
    if s > 0:
        gamma_full = [0.1,0.25,0.4,0.55,0.7,0.85]
        theta_full = [np.pi/4,-np.pi/4]*3
        gamma = gamma_full[:s]
        theta = theta_full[:s]
        for i in range(s):
            J[r+2*i,r+2*i] = gamma[i]*np.cos(theta[i])
            J[r+2*i,r+2*i+1] = gamma[i]*np.sin(theta[i])
            J[r+2*i+1,r+2*i] = -gamma[i]*np.sin(theta[i])
            J[r+2*i+1,r+2*i+1] = gamma[i]*np.cos(theta[i])
    ...
    for t in range(T+burn):
        eps[:,t] = np.random.normal(loc=0.0, scale=1, size=N)
        y[:,t] = Phi @ y[:,t-1] - Theta @ eps[:,t-1] + eps[:,t]
    return (y[:,burn:],lmbd,gamma,theta,G,L,Phi,Theta)
