...
from dgp import DGP_VAR,DGP_VARMA,DGP_SEASON_VARMA,DGP_SEASON_VAR_SOFT,DGP_MIX,DGP_SEASON_VAR_BIC,DGP_MIX_BIC,DGP_VARMA_BIC
...
def main():
    ...
    parser.add_argument('--A_init',type=str,choices=['spec','true','zero','GD','noisetrue'],default='zero')
    ...
    if args.task == 'convergence':
        args.max_iter = 200; args.stop_method = 'none'
    ...
    for rep in tqdm(range(args.n_rep)):
        ...
        if args.dgp == 'ar':
            ...
            y,true_A = DGP_SEASON_VAR_SOFT(args.N,args.T,burn=200,r=args.dgp_r,rho=args.rho)
        elif args.dgp == 'mix':
        ...
        if args.iterative:
            ...
            loss_list[rep] = loss
            mean_err_path += err_path; mean_log_est_path += np.log(est_path/est_path[0])
            # print(A-true_A[:,:,:P])
    ...
    if args.task == 'convergence':
        with open('{}/csv/p{}r{}T{}N{}.csv'.format(wkdir,args.dgp_p,args.dgp_r,args.T,args.N),'a') as f:
            f.write(f'{args.n_rep},T0{args.T0}_N{args.N}_s{args.s}_A_init_{args.A_init},{mean_log_est_path}\n')
