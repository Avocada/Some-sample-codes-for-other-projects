...
if args.dgp == 'arma':
    if args.rho == 0.7 and args.dgp_r == 4:
        T_list = [500]*30
        T0_list = [20,100,200]*10
        base_s_list = [10,10,10]
    ...
    if args.rho == 0.7 and args.dgp_r == 4:
        T_list = [800]*24
        T0_list = [25,50,100,200]*6
        base_s_list = [10,10,10,10]
        s_list = list(chain.from_iterable([[i+j for i in base_s_list] for j in range(-8,-2)]))
        # s_list = [8,8,8,8]
if args.task == 'rate':
    exp_name = f'NewDGP_400_{args.dgp}_optimal_s_change_T0_N20'
...
# rate
    for T,T0,s in zip(T_list,T0_list,s_list):
        ...
        for init in ['true']:
            command = ['python3.9','../train.py','--dgp',args.dgp,'--season','4','--dgp_p',str(args.dgp_p),'--dgp_r',str(args.dgp_r),'--rho',str(args.rho),'--T',str(T),'--s',str(s),'--T0',str(T0),'--A_init',init,'--exp_name',exp_name,'--task',args.task,'--stop_method','loss','--schedule','none','--lr','1e-3','--n_rep',str(args.n_rep),'--N','20']
            print(' '.join(command))
            subprocess.Popen(command,stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
